"""
TOOL CONTRACT — graphs.analysis_plots
════════════════════════════════════════════════════════════════

IDENTITY
--------
name          : graphs.analysis_plots
domain        : descriptive
version       : 1.0.0
...
DESCRIPTION
-----------
Génère les graphiques descriptifs du portefeuille : pyramide des âges
(avec ou sans ventilation H/F), série temporelle annuelle (exposition et
décès), et répartitions catégorielles (camemberts). Retourne une image
PNG encodée en base64.

WHEN TO USE
-----------
Appeler après les outils statistical_analysis correspondants pour visualiser
leurs résultats. Intégrer dans le rapport descriptif PDF ou afficher dans
l'interface. Proposer proactivement après portfolio_summary, age_distribution,
time_series, ou segmentation.

WHEN NOT TO USE
---------------
Ne pas appeler "age_pyramid" sans ages disponible dans le data_store (appeler
statistical_analysis.age_distribution d'abord).

PREREQUISITES
-------------
required_tools: [varies by chart]
  - age_pyramid → statistical_analysis.age_distribution
  - time_series → statistical_analysis.time_series
  - segmentation → statistical_analysis.segmentation
required_data_store_keys: [varies by chart — ages | series | segmentation]
...
params:
  ...
  title_suffix:
    ...
    values  : texte libre
    default : ""
    note    : Texte ajouté au titre du graphique.
  by_sex:
    type    : bool
    values  : true | false
    default : false
    note    : Si true, affiche la pyramide séparée H/F.
              Requiert distribution_H et distribution_F dans ages.
...
ERROR HANDLING
--------------
error: "Données ages manquantes dans data store. Appeler statistical_analysis.age_distribution d'abord."
  → cause  : ages absent du data_store.
  → action : Appeler statistical_analysis.age_distribution avant de relancer.
error: "chart inconnu : '...'"
  → cause  : Valeur de chart incorrecte.
  → action : Utiliser uniquement : age_pyramid, time_series, segmentation.

AGENT GUIDANCE
--------------
reasoning_hint: >
  Proposer systématiquement les graphiques descriptifs après chaque outil
  statistical_analysis. age_pyramid après age_distribution, time_series après
  time_series, segmentation après segmentation.
exemplar_query: >
  Quels graphiques descriptifs proposer en début d'analyse actuarielle ?

CATALOGUE METADATA
------------------
display_name      : Graphiques descriptifs du portefeuille
short_description : Génère pyramide des âges, séries temporelles et répartitions catégorielles.
domain            : descriptive
capability_group  : graphs
depends_on        : [statistical_analysis.age_distribution, statistical_analysis.time_series, statistical_analysis.segmentation]
required_by       : []
client_visible    : true
"""
