...
Résumé global du portefeuille : volume, période, exposition, décès, âges, qualité des données.
...
Colonnes du DataFrame (détectées automatiquement via column_schema) :
  ...
  Optionnelles :
    • date_sortie      (rôle : date_sortie)    — date de sortie → exposition calculée
    • date_naissance   (rôle : date_naissance) — date de naissance → âge calculé à l'entrée
    • duree_obs_ans    (rôle : duree_obs_ans)  — durée d'observation pré-calculée en années
                                                 (prioritaire sur le calcul depuis les dates)
    • sexe             (rôle : sexe)           — H/F → répartition par sexe

Paramètres (params dict) :
    observation_end          : str | datetime — date de fin d'observation pour tronquer
                                                les dates de sortie futures (ex : "2024-12-31")
    max_reasonable_exit_date : str            — date max acceptable si observation_end absent
                                                (défaut : "2100-01-01")
...
Clés toujours présentes :
    nb_contrats                    : int    — nombre total de lignes
    nb_lignes_total                : int    — idem
    colonnes                       : list   — liste des colonnes du DataFrame
    nb_deces                       : int | None
    proportion_deces_lignes        : float | None  — nb_deces / nb_contrats
    exposition_totale_pa           : float | None  — total personne-années
    exposition_moyenne_pa_par_contrat : float | None
    taux_brut_deces_par_pa         : float | None  — nb_deces / exposition
    taux_brut_deces_pour_1000_pa   : float | None
    source_exposition              : str | None    — "colonne_duree_obs_ans" | "calculee_depuis_dates" | None
    source_age                     : str | None    — "calcule_depuis_date_naissance_et_entree" | None
    qualite_donnees                : dict   — compteurs d'anomalies (dates invalides, durées négatives…)
    warnings                       : list   — messages d'alerte en langage naturel

Clés conditionnelles (si colonnes présentes) :
    date_entree_min / date_entree_max  : str  — période couverte
    date_sortie_min / date_sortie_max  : str
    age_min / age_max / age_moyen / age_median / age_q25 / age_q75 : float
    repartition_sexe                   : dict  — {H: n, F: n, AUTRE: n}
    repartition_sexe_pct               : dict
════════════════════════════════════════════════════════════════
